...
class Patient extends CI_Controller
{
            ...
            function manage_email_sent($param1 = '', $param2 = '', $param3 = '')
	{
		if ($this->session->userdata('patient_login') != 1)
			redirect(base_url() . 'index.php?login', 'refresh');
                if($param1 == 'delete_email' && $param2 == 'sent')
                {
                  ...
                  redirect(base_url() . 'index.php?patient/manage_email_sent', 'refresh');
                }
                
               
           
                $this->db->where(array('user_from_type'=>'8', 'user_from_id'=>$this->session->userdata('patient_id')));
		...
		$page_data['page_name']  = 'manage_email_sent';
                $page_data['patient_id'] = $this->session->userdata('patient_id');
		...
		if ($this->session->userdata('patient_login') != 1)
			redirect(base_url() . 'index.php?login', 'refresh');
                ...
                if($param1 == 'delete_email' && $param2 == 'inbox')
                {
                  $this->db->where('message_id',$param3);
                  $this->db->delete('message');
                  redirect(base_url() . 'index.php?patient/manage_email', 'refresh');
                }
                
		
                $this->db->where(array('user_to_type'=>'8', 'user_to_id'=>$this->session->userdata('patient_id')));
                $this->db->order_by('message_id' , 'desc'); 
	...
	// visit details for patient
	function visit_details($param1 = '', $param2 = '', $param3 = '')
	{
	if ($this->session->userdata('patient_login') != 1)
			redirect(base_url() . 'index.php?login', 'refresh');
	...
	if($param1 == 'create')
	{
	$data['patient_type'] = $this->input->post('patienttype');
	$data['patient_id'] =  $this->session->userdata('patient_id');
	$data['last_visit_date'] = $this->input->post('lastvisitdate');
	$data['purpose_of_visit'] = $this->input->post('pov');
	$this->db->insert('visit_details', $data);
	$this->session->set_flashdata('flash_message', get_phrase('Last Visited Date Added'));	
	redirect(base_url() . 'index.php?patient/visit_details', 'refresh');
	}
	$page_data['page_name']	= 'visit_details';
	$page_data['page_title']   = get_phrase('patient visit details');
	$page_data['visit_details'] = $this->db->get_where('visit_details', array(
	'patient_id'=> $this->session->userdata('patient_id')
	))->result_array();
	$this->load->view('index',$page_data);	
	...
	function antenatal($param1 = '', $param2 = '', $param3 = '')
	{
	if ($this->session->userdata('patient_login') != 1)
			redirect(base_url() . 'index.php?login', 'refresh');
	...
	$data['patient_id'] = $this->session->userdata('patient_id');
	$data['reg_time'] = strtotime(date('Y-m-d') . ' ' . date('H:i:s'));
		$this->db->insert('antenatal', $data);

			$this->session->set_flashdata('flash_message', get_phrase('antenatal registration successful'));
			
			redirect(base_url() . 'index.php?patient/antenatal', 'refresh');	
	}
			
	$page_data['page_name']      = 'antenatal';
	$page_data['page_title']     = get_phrase('antenal_registration');
	$page_data['edit_profile'] = $this->db->get_where('patient', array(
		'patient_id' => $this->session->userdata('patient_id')
		))->result_array();
		$page_data['antenatal'] = $this->db->get_where('antenatal', array(
		'patient_id' => $this->session->userdata('patient_id')
		))->result_array();
		$this->load->view('index', $page_data);			
	}
